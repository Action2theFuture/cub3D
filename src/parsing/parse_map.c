...
static char *format_line(t_description_file *desc_file, char *map_line, char *new_line)
{
    int i;
    i = 0;
...
{
    if (x < 0 || y < 0 || x > desc_file->map_width - 1 || y > desc_file->map_height - 1 || desc_file->map[y][x] == '1')
        return;
    if (desc_file->map[y][x] == '0')
    {
        *is_map_not_enclosed = true;
        return;
    }
    if (desc_file->map[y][x] == 32)
        desc_file->map[y][x] = '1';
