...
/*   Updated: 2024/11/01 21:23:41 by max              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void raycast(t_data *data)
{
    ...
    {
        ray_angle = data->player.view_angle + (x / (float)data->screen_size_x - 0.5f) * FOV_DEGREE;
        ray_angle = degrees_to_radians(ray_angle);
        ray_vector_x = cos(ray_angle);
        ray_vector_y = -sin(ray_angle);
        // if (x % 100 == 0 || x == 960 || x == 1919)
        //     printf("%d -> ray x = %f  ray y = %f\n", x, ray_vector_x, ray_vector_y);
        if (x == 1919)
            test_dda(data, ray_vector_x, ray_vector_y);
